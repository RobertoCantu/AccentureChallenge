...
export const useWorkspace = (folderId?: string) => {
	...
	const addNote = (fileName: string, file: File | any) => {
		...
		createFolder(folderName, currentFolderId)
			.then((folder) => {
				if (folder) {
					...
					console.error(`Ya existe una carpeta con el nombre ${folderName}`);
				}
			})
			.catch(setError);
	};
		...
		FileService.updateFile(fileId, updates, file)
			.then((file) => {
				if (file) {
					...
					const updateIdx = fileItems.findIndex((file) => file.id === fileId);
